        ...
        <div className={styles.header}>
          ...
          <div className={styles.iconSection}>
            {name === 'To Do' && (
              ...
              >
                <img src={plus} height={20} width={20} alt="Add Cards" />
              </button>
            )}
            <button
              className={styles.actionButtons}
              onClick={() => setAreAllCardsCollapsed(true)}
            >
              <img src={CollapseAll} height={20} width={20} alt="CollapseAll" />
            </button>
          </div>
        </div>
        {task &&
          task.tasks &&
          task.tasks.map((children, index) => (
            <Card
              key={`${children}-${index}`}
              boardName={name}
              taskID={children}
              ...
              areAllCardsCollapsed={areAllCardsCollapsed}
            />
          ))}
      </div>
    </div>
